...
def noise_torch(clean_signal, snr):
    """
    Add noise to a clean signal based on the given SNR (Signal-to-Noise Ratio).

    Args:
        clean_signal (torch.Tensor): Clean signal (data_num, 2, num_antennas).
        snr (float): Signal-to-Noise Ratio (linear scale, not dB).

    Returns:
        torch.Tensor: Noisy signal with the same shape as the clean signal.
    """
    # Ensure input is a torch tensor
    if not isinstance(clean_signal, torch.Tensor):
        clean_signal = torch.from_numpy(clean_signal).float()

    # Get dimensions directly from tensor shape
    data_num, _, num_antennas = clean_signal.shape

    # Flatten real and imaginary parts for easier manipulation
    clean_real = clean_signal[:, 0, :]  # Real part
    clean_imag = clean_signal[:, 1, :]  # Imaginary part

    # Compute the power of the clean signal
    signal_power = torch.mean(torch.pow(clean_real, 2) + torch.pow(clean_imag, 2), dim=1)

    # Convert SNR to noise power (SNR is in linear scale)
    noise_power = signal_power / snr

    # Generate Gaussian noise (for both real and imaginary parts)
    noise_real = torch.randn(data_num, num_antennas, device=clean_signal.device) * torch.sqrt(noise_power[:, None] / 2)
    noise_imag = torch.randn(data_num, num_antennas, device=clean_signal.device) * torch.sqrt(noise_power[:, None] / 2)

    # Add noise to the clean signal
    noisy_real = clean_real + noise_real
    noisy_imag = clean_imag + noise_imag

    # Combine noisy real and imaginary parts back together
    noisy_signal = torch.zeros_like(clean_signal)
    noisy_signal[:, 0, :] = noisy_real
    noisy_signal[:, 1, :] = noisy_imag
